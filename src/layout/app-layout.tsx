...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administradorda-faculdade', 'professor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<UniversityInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/universities` : null),
    () => getUniversities({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'University',
      path: isTenantUser ? `/universities/view/${data?.[0]?.id}` : '/universities',
      entity: 'university',
    ...
    { name: 'Student', path: '/students', entity: 'student', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
