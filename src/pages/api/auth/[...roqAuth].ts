...
export default RoqAuth({
  hooks: {
    ...
    onRegisterSuccess: async ({ user }) => {
      ...
      if (user.roles.data.some((role) => ['Administradorda Faculdade'].includes(role.name))) {
        const tenant = await roqClient.asSuperAdmin().tenant({ id: user.tenantId });
        const { name: tenantName } = tenant.tenant ?? {};
        const mainEntity = await prisma.university.create({
          data: {
            tenant_id: user.tenantId,
            user_id: owner.id,
            name: tenantName,
        ...
        overrides['university'] = [mainEntity];
      }
